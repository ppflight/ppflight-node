...
// LiveView exposes the current tracker snapshot for local stats APIs.
func (t *Tracker) LiveView() (online map[int]int, traffic map[int][2]int64, alive map[int][]string, connCount, inSpeed, outSpeed int64) {
	s := t.live.Load()
	online = make(map[int]int, len(s.online))
	for uid, n := range s.online {
		online[uid] = n
	}
	traffic = copyTrafficMap(s.traffic)
	...
	return online, traffic, alive, int64(s.connCount), s.inSpeed / 10, s.outSpeed / 10
}
