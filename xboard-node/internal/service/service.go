...
import (
	...
	"github.com/ppflight/ppflight-node/xboard-node/internal/cert"
	"github.com/ppflight/ppflight-node/xboard-node/internal/cert/dnsproviders"
	"github.com/ppflight/ppflight-node/xboard-node/internal/config"
	"github.com/ppflight/ppflight-node/xboard-node/internal/controlplane"
	"github.com/ppflight/ppflight-node/xboard-node/internal/kernel"
	"github.com/ppflight/ppflight-node/xboard-node/internal/kernel/singbox"
	"github.com/ppflight/ppflight-node/xboard-node/internal/kernel/xray"
	"github.com/ppflight/ppflight-node/xboard-node/internal/limiter"
	"github.com/ppflight/ppflight-node/xboard-node/internal/model"
	"github.com/ppflight/ppflight-node/xboard-node/internal/monitor"
	"github.com/ppflight/ppflight-node/xboard-node/internal/nlog"
	"github.com/ppflight/ppflight-node/xboard-node/internal/stats"
	"github.com/ppflight/ppflight-node/xboard-node/internal/tracker"
)
...
func (s *Service) publishStats() {
	online, traffic, alive, connCount, inSpeed, outSpeed := s.tracker.LiveView()
	...
	uuidByID := make(map[int]string, len(users))
	for _, u := range users {
		uuidByID[u.ID] = u.UUID
	}
	...
	views := make([]stats.UserView, 0, len(ids))
	onlineUsers := 0
	for _, id := range ids {
		devices := online[id]
		if devices > 0 {
		...
		tr := traffic[id]
		views = append(views, stats.UserView{
			UserID:        id,
			UUID:          uuidByID[id],
	...
	stats.Update(stats.NodeView{
		NodeID:            s.cfg.Panel.NodeID,
		InstanceID:        s.cfg.InstanceID,
		Protocol:          protocol,
		Port:              port,
		ActiveConnections: int(connCount),
		UploadSpeedBps:    outSpeed,
		DownloadSpeedBps:  inSpeed,
		Users:             views,
